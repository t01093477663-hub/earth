...
with col2:
    ...
    if "내행성" in target_mode:
        orbit_radius = 16
        # [오류 교정]: 반시계 방향(서->동) 공전 각도 좌표계 완벽 수정
        # 지구(6시 방향 고정)에서 태양을 바라볼 때 오른쪽이 동방최대이각(47°), 왼쪽이 서방최대이각(313°)
