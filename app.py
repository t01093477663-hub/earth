...
# 시간이 변함에 따라 태양과 천체의 위치 관계 각도(이각)를 시뮬레이션
time_progress = (selected_hour / 24.0) * 2 * np.pi  # 시간에 따른 일주 각도

if target.startswith("달"):
    # 달은 시간에 따라 위치가 바뀌며, 매일 위상이 달라짐 (기본 음력 7일 기준에서 시간 흐름 반영)
...
if 6 <= selected_hour < 11:
    current_sky = "linear-gradient(180deg, #74ebd5 0%, #ACB6E5 100%)" # 아침
    is_visible = False if target.startswith("화성") else True
    sky_status = "☀️ 낮 시간대로 태양빛에 의해 관측이 어렵습니다."
elif 11 <= selected_hour < 17:
    current_sky = "linear-gradient(180deg, #4facfe 0%, #00f2fe 100%)" # 낮
    is_visible = False
    sky_status = "☀️ 태양이 중천에 떠 있어 천체가 가려집니다."
elif 17 <= selected_hour < 20:
    current_sky = "linear-gradient(180deg, #ff9a9e 0%, #fecfef 99%, #feada6 100%)" # 저녁
    is_visible = True
    sky_status = "🌅 개기일몰 및 초저녁 관측 가능 시간대입니다."
else:
    current_sky = "linear-gradient(180deg, #050505 0%, #111122 100%)" # 밤/새벽
    # 내행성은 밤 22시~새벽 2시 사이에는 지평선 아래로 내려가서 안 보임 특징 반영
...
with col1:
    ...
    # 안전하게 결합된 HTML 블록 출력 (텍스트 에러 완전 해결)
    sky_html = f"""
...
# 6. 우측 요약 대시보드 및 이미지 가이드
with col2:
    st.markdown('<div class="info-card">', unsafe_allow_html=True)
    st.subheader("🪐 공간적 기하 관계 이해")
    st.write("지구과학 학습을 위해 태양-지구-천체의 공전 궤도상 위치 관계를 나타낸 구조 기하도입니다.")
    
    # 내합/외합, 충/구 관계를 한눈에 볼 수 있는 과학 다이어그램 배치
    if target.startswith("금성"):
        st.write("**내행성 위치 관계도 (내합·외합·최대이각)**")
        st.image("", caption="태양 중심 내행성 기하학적 배치도")
    elif target.startswith("화성"):
        st.write("**외행성 위치 관계도 (충·구·합)**")
        st.image("", caption="태양 중심 외행성 기하학적 배치도")
    else:
        st.write("**태양-지구-달 위상 관계도**")
        st.image("", caption="달의 공전 위치별 위상 변화 가이드")
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="info-card">', unsafe_allow_html=True)
    st.subheader("📊 실시간 데이터 요약")
    st.write(f"• **현재 시각:** {selected_hour:02d}:00 시각 적용")
    st.write(f"• **관측 고도 상태:** {'관측 최적기' if is_visible else '관측 불가능 (지평선 아래 또는 태양빛 간섭)'}")
    st.markdown('</div>', unsafe_allow_html=True)
